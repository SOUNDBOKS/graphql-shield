...
describe('caching:', () => {
  ...
  test('Contextual Cache - rules get executed only once if reused.', async () => {
    ...
    expect(ruleOneMock).toHaveBeenCalledTimes(1);
    expect(ruleTwoMock).toHaveBeenCalledTimes(1);
  });
  ...
  test('No Cache - rule is reexecuted every time.', async () => {
    ...
    expect(allowMock).toHaveBeenCalledTimes(5);
  });
